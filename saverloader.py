...
def load(ckpt_dir, model, optimizer=None, scheduler=None, model_ema=None, step=0, model_name='model', ignore_load=None,
    ...
    # torch.load's map_location must be a str / torch.device / dict / callable -- never a list.
    # Several call sites pass device_ids (a list) here; coerce it to a usable device string.
    if isinstance(device, (list, tuple)):
        device = ('cuda:%d' % int(device[0])) if torch.cuda.is_available() and len(device) else 'cpu'
    ...
    if not os.path.exists(ckpt_dir):
        ...
        if len(ckpt_names) > 0:
            ...
            if ignore_load is not None:
                ...
                checkpoint = torch.load(path, map_location=device, weights_only=False)['model_state_dict']
