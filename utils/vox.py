...
class Vox_util(object):
    ...
    def get_ref_T_mem(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        ref_T_mem = mem_T_ref.float().inverse()  # ← 여기가 핵심!
    ...
    def voxelize_xyz_and_feats_voxelnet(self, xyz_ref, feats, Z, Y, X, already_mem=False, assert_cube=False,
                                        clean_eps=0, use_pseudolidar_occupancy_map=False):
        B, N, D = list(xyz_ref.shape)  # point coords: B, N=num_of_points, D=3
        B2, N2, D2 = list(feats.shape)  # point features: B, N2=num_of_points, D2=16 -> dims of metadata
        assert (D == 3)
        assert (B == B2)
        ...
        is_pointnet = False
        if is_pointnet:
            pseudolidar_point_cloud_feats_filtered = self.get_voxelnet_dense_feature_voxels(
                xyz_mem, feats, Z, Y, X, clean_eps=clean_eps, xyz_zero=xyz_zero)

            return pseudolidar_point_cloud_feats_filtered
    ...
    def get_feat_occupancy(self, xyz, feat, Z, Y, X, clean_eps=0, xyz_zero=None, is_voxelnet=False):
        ...
        # New store pseudolidar points in voxels
        if is_voxelnet:
            """ Work in Progress """
            ...
            # T=10
            voxel_input_buffer = torch.zeros((B, 35000, 10, 3 + feat.shape[-1]), device=x.device)
            voxel_coord_buffer = torch.zeros((B, 35000, 3), device=x.device)
            ...
            pseudolidar_point_cloud = torch.zeros_like(xyz)
            pseudolidar_point_cloud[:, :, 0] = x
            pseudolidar_point_cloud[:, :, 1] = y
            pseudolidar_point_cloud[:, :, 2] = z

            voxel_indices = pseudolidar_point_cloud.float().long()
    ...
    def get_voxelnet_dense_feature_voxels(self, xyz, feat, Z, Y, X, clean_eps=0, xyz_zero=None,
        ...
        # for pointnet
        pseudolidar_point_cloud_filtered = torch.zeros_like(xyz)
        pseudolidar_point_cloud_filtered[:, :, 0] = x
        pseudolidar_point_cloud_filtered[:, :, 1] = y
        pseudolidar_point_cloud_filtered[:, :, 2] = z

        # ZYX fromat  -> actually correct
        # pseudolidar_point_cloud_filtered[:, :, 0] = z
        # pseudolidar_point_cloud_filtered[:, :, 1] = y
        # pseudolidar_point_cloud_filtered[:, :, 2] = x

        pseudolidar_point_cloud_feats_filtered = torch.cat([pseudolidar_point_cloud_filtered, feat], dim=2)
        pseudolidar_point_cloud_feats_filtered_np = pseudolidar_point_cloud_feats_filtered.detach().cpu().numpy()
        ...
        # pseudolidar_point_cloud[:, :, 2] = z
        # in ZYX Format
        pseudolidar_point_cloud[:, :, 0] = z
        pseudolidar_point_cloud[:, :, 1] = y
        pseudolidar_point_cloud[:, :, 2] = x

        voxel_indices = torch.round(pseudolidar_point_cloud).int()
        ...
        # pseudolidar_occupancy_map = mask
        # (B, 3500, 10, 19) , (B,3500, 3)
        return voxel_input_buffer, voxel_coord_buffer, number_off_occupied_voxels  # , pseudolidar_occupancy_map
