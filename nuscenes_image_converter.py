...
from depth.test_simple_full import test_simple #, parse_args
...
#         #     print(f" new_imgname {new_imgname}")
...
#         # ''' make depth img for depth estimation branch '''
#         for cam in cams:
#             samp = nusc.get('sample_data', sample['data'[cam]])
#             cam_name = cam
#             cam_dir = os.path.join(samples_dir, cam_name)
...
#             img = Image.open(imgname)
#             print(f"new_imgname {img}")
#             test_simple(cam_dir,)

#             #args = parse_args()
#             #img_scaled = img.resize(size=final_dim, resample=Image.N)

#         sample_num += 1
#     return


def convert_depth_imgs(version, dataroot, is_train, target_dir):
    # load nuscenes
    nusc = NuScenes(version='v1.0-{}'.format(version), dataroot=dataroot, verbose=True)
    print("... data loaded! \n")
    ...
    #res_str = str(final_dim[0]) + "_" + str(final_dim[1])
    dept_dir = "dept_dir"
    dept_dir = os.path.join(target_dir, dept_dir)
    ...
    global dept_sample_dir

    for cam in cams:
        dept_sample_dir = os.path.join(dept_dir,cam)
        if not os.path.exists(dept_sample_dir):
            os.mkdir(dept_sample_dir)
            #print(f"cam_dir cam_dircam_dircam_dircam_dir {cam}")
    ...
    for sample in samples:
        ...
        for cam in cams:
            ...
            test_simple(img,dept_sample_dir)
...
if __name__ == '__main__':
    ...
    version = "mini"  # specifies the split that is loaded
    print("Version for conversion: '%s' " % version)
    dataroot = "/root/data/dataset/nuscenes_datasets/nuscenes_datasets/nuscenes/"  # path to NuScenes directory
    print("Data is taken from '%s'" % dataroot)
    target_dir = "/root/data/dataset/nuscenes_datasets/nuscenes_datasets/nuscenes/"   # custom data path
    ...
    # you want to change rgb img to dpeth img with RT_MonoDepth
    convert_depth_imgs(version=version,
                        dataroot=dataroot,
                        is_train=True,
                        target_dir=target_dir)