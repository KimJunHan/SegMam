...
def _postproc_drivable_mask(pred_bin_bhw: torch.Tensor) -> torch.Tensor:
    """Eval-time post-processing for the binarized drivable-area prediction (B,H,W bool).
    Gentle 3x3 morphological close (dilate then erode) — fills sub-2-pixel gaps and smooths
    the boundary without merging legitimately-separated drivable components or filling large
    enclosed non-drivable regions. Earlier variant (fill_holes + keep largest component)
    was tested and lost ~6 drivable IoU because drivable is often multi-component in the
    100x100m BEV crop and contains legitimate enclosed non-drivable regions (e.g. roundabout
    centers, building blocks surrounded by road); morph-close is the safe alternative.
    Enabled via env var SEGMAM_DRIVABLE_POSTPROC=1 (default off => original behavior)."""
    try:
        from scipy import ndimage as _ndi
    except Exception:
        return pred_bin_bhw
    se = np.ones((3, 3), dtype=bool)
    out = pred_bin_bhw.clone()
    arr = pred_bin_bhw.detach().cpu().numpy().astype(bool)
    for b in range(arr.shape[0]):
        m = arr[b]
        if m.any():
            m = _ndi.binary_dilation(m, structure=se)
            m = _ndi.binary_erosion(m, structure=se)
        out[b] = torch.from_numpy(m).to(out.device)
    return out
...
def run_model(model, loss_fn, map_seg_loss_fn, d, device='cuda:0', sw=None, use_pseudolidar_encoder=None,
    ...
    if pseudolidar_encoder_type == "voxel_net":
        imgs, rots, trans, intrins, seg_bev_g, \
            ...
            number_of_occupied_voxels, *_ = d        # ← *_ 추가
        ...
        imgs, rots, trans, intrins, seg_bev_g, \
            valid_bev_g, pseudolidar_data, bev_map_mask_g, bev_map_g, egocar_bev = d
    ...
    rad_data = pseudolidar_data.to(device).permute(0, 2, 1)  # B, R, 19
    xyz_rad = rad_data[:, :, :3]
    meta_rad = rad_data[:, :, 3:]
    ...
    if isinstance(seg_e, tuple):
        seg_e, stage_timings = seg_e
    else:
        stage_timings = {}
    ...
    if train_task == 'both' or train_task == 'map':
        ...
        pred_bin = (torch.sigmoid(bev_map_mask_e) >= map_seg_threshold).bool()  # (B, n_map_classes, H, W)
        # Eval-time post-processing on the drivable-area channel only (index 0 in iou_labels).
        # Applied to drivable only — fill-holes / largest-component would HURT thin classes
        # (road_divider, lane_divider). No effect unless SEGMAM_DRIVABLE_POSTPROC=1.
        if os.environ.get("SEGMAM_DRIVABLE_POSTPROC", "0") == "1" and pred_bin.shape[1] > 0:
            pred_bin = pred_bin.clone()
            pred_bin[:, 0] = _postproc_drivable_mask(pred_bin[:, 0])
        tp = (pred_bin & bev_map_mask_g.bool()).sum(dim=[2, 3])
        fp = (pred_bin & ~bev_map_mask_g.bool()).sum(dim=[2, 3])
        fn = (~pred_bin & bev_map_mask_g.bool()).sum(dim=[2, 3])
        ...
        # ######
        # Finer grid around 0.40 (where drivable typically peaks) — 12 thresholds kept to match
        # the hard-coded shape of map_masks_multi_ious_*; trimmed the very-low/very-high
        # values (rarely optimal) and added 0.38/0.42/0.44/0.46 for ~2%-step search near 0.40.
        map_seg_thresholds = torch.Tensor([0.30, 0.35, 0.38, 0.40, 0.42, 0.44, 0.46, 0.50, 0.55, 0.60, 0.70, 0.80]).to(device)
    ...
    return total_loss, metrics, inference_t, stage_timings
...
def main(
        ...
        log_dir='logs_eval_nuscenes_segmam',
        ...
        use_pseudolidar_occupancy_map=False,  # occupancy_pseudolidar_rpn
    ...
    while global_step < max_iters:
        ...
        with torch.no_grad():
            total_loss, metrics, inference_t, stage_timings = run_model(
                model, seg_loss_fn, map_seg_loss_fn, sample, device, sw_ev,
                use_pseudolidar_encoder, pseudolidar_encoder_type, train_task,
                use_shallow_metadata=use_shallow_metadata,
                use_obj_layer_only_on_map=use_obj_layer_only_on_map)
            inference_time += inference_t
            if stage_timings:
                # skip the first iter to ignore CUDA warmup
                if global_step > 1:
                    for k, v in stage_timings.items():
                        stage_timings_sum[k] = stage_timings_sum.get(k, 0.0) + float(v)
                    stage_timings_count += 1
    ...
    if stage_timings_count > 0:
        print("\n=== Mean per-stage timing (ms), averaged over %d iters (warmup excluded) ===" % stage_timings_count)
        for k in sorted(stage_timings_sum.keys()):
            print("  %-40s %.3f ms" % (k, stage_timings_sum[k] / stage_timings_count))
    writer_ev.close()
