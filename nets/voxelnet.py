...
# https://github.com/TUMFTM/PseudolidarVoxelFusionNet  [2]
# code modified to fit the desired dimensions use case (pseudolidar backbone not a full 3D object detection net
...
class SVFE(nn.Module):
    """
    Stacked VFE with per-layer skip connections.
      - skip_mode='add'   : y = VFE(x) + alpha * FCN(x)
      - skip_mode='concat': y = Reduce( concat([VFE(x), FCN(x)]) )
    Shapes:
      x: (B, K, T, Cin) with mask: (B, K, T)
      VFE1: Cin=4 -> 32, VFE2: 32 -> 128 (point-wise features)
    """

    def __init__(self, skip_mode: str = "add", gated: bool = True):
        super(SVFE, self).__init__()
        assert skip_mode in ("add", "concat")
        ...
        # Skip projections (point-wise)
        # raw(4) -> 32 to match VFE1, 32 -> 128 to match VFE2
        self.skip1_proj = FCN(4, 32)
        self.skip2_proj = FCN(32, 128)

        # Concat -> reduce (if concat mode)
        if self.skip_mode == "concat":
            # concat channels: (32 + 32) -> 32, (128 + 128) -> 128
            self.reduce1 = FCN(32 + 32, 32)
            self.reduce2 = FCN(128 + 128, 128)

        # Optional learnable gates (alpha)
        if self.gated and self.skip_mode == "add":
            self.alpha1 = nn.Parameter(torch.tensor(1.0))
            self.alpha2 = nn.Parameter(torch.tensor(1.0))
        else:
            # buffers so that .to(device) works consistently
            self.register_buffer("alpha1", torch.tensor(1.0))
            self.register_buffer("alpha2", torch.tensor(1.0))

        # Final head (point-wise) then max-pool over T
        self.head = FCN(128, 128)

    def forward(self, x):
        """
        x: (B, K, T, C_in=4)
        returns: (B, K, 128) after point-wise aggregation (max over T)
        """
        B, K, T, _ = x.shape
        # valid mask: (B,K,T)
        mask = torch.ne(torch.max(x, -1)[0], 0)
        mask4_32  = mask.unsqueeze(3).expand(B, K, T, 32)
        mask4_128 = mask.unsqueeze(3).expand(B, K, T, 128)

        # ----------------
        # Block 1 (4 -> 32)
        # ----------------
        x1_vfe  = self.vfe_1(x, mask)          # (B,K,T,32), 내부에서 mask 적용됨
        x1_skip = self.skip1_proj(x)            # (B,K,T,32)  (mask 미적용 경로)
        x1_skip = x1_skip * mask4_32.float()    # invalid point zero-out

        if self.skip_mode == "add":
            y1 = x1_vfe + self.alpha1 * x1_skip
        else:  # concat
            y1 = torch.cat([x1_vfe, x1_skip], dim=-1)   # (B,K,T,64)
            y1 = self.reduce1(y1)                       # (B,K,T,32)
            y1 = y1 * mask4_32.float()

        # ----------------
        # Block 2 (32 -> 128)
        # ----------------
        x2_vfe  = self.vfe_2(y1, mask)          # (B,K,T,128)
        x2_skip = self.skip2_proj(y1)            # (B,K,T,128)
        x2_skip = x2_skip * mask4_128.float()

        if self.skip_mode == "add":
            y2 = x2_vfe + self.alpha2 * x2_skip  # (B,K,T,128)
        else:  # concat
            y2 = torch.cat([x2_vfe, x2_skip], dim=-1)  # (B,K,T,256)
            y2 = self.reduce2(y2)                      # (B,K,T,128)
            y2 = y2 * mask4_128.float()

        # ----------------
        # Head + masking + max-pool over points
        # ----------------
        y2 = y2 * mask4_128.float()
        y2 = self.head(y2)                              # (B,K,T,128)
        y2 = y2 * mask4_128.float()
        out = torch.max(y2, dim=2)[0]                   # (B,K,128)
...
class RPN(nn.Module):
    def __init__(self, output_dim):
        ...
        if self.occupancy_pseudolidar_rpn:
...
class VoxelNet(nn.Module):
    ...
    def __init__(self, use_col=False, reduced_zx=False, output_dim=128, use_pseudolidar_occupancy_map=False):
        ...
        if self.use_pseudolidar_occupancy_map:
    ...
    def voxel_indexing(self, sparse_features, coords, number_of_occupied_voxels, dinovoxel=None):
        ...
        if dinovoxel is not None:  # if we get voxelized dino features
            ...
            # the simplest method: replace voxels in dinovoxel where we have encoded pseudolidar information
    ...
    def forward(self, voxel_features, voxel_coords, number_of_occupied_voxels, dinovoxel=None):
        ...
        vwfs, pseudolidar_occupancy_map = self.voxel_indexing(vwfs, voxel_coords, number_of_occupied_voxels,
        ...
        if self.use_col:  # if using rpn layers
            pseudolidar_feats = self.rpn(cml_out)  # cfg.N,-1,cfg.H, cfg.W))
        else:
            if self.output_dim != 128 and False:
                cml_out = self.fit_adapter(cml_out)
            cml_out_dims = cml_out.clone().requires_grad_(True)
            pseudolidar_feats = cml_out_dims

        if self.use_pseudolidar_occupancy_map:
            pseudolidar_feats = self.occ_voxel_fuse(occ=pseudolidar_occupancy_map, x=pseudolidar_feats)

        return pseudolidar_feats
