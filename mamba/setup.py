...
if not SKIP_CUDA_BUILD:
    ...
    if HIP_BUILD:
        ...
        if bare_metal_version >= Version("11.8"):
            ...
            cc_flag.append("arch=compute_90,code=sm_90")
                # CUDA 12.3+ → Blackwell (sm_100)
        if bare_metal_version >= Version("12.3"):
            cc_flag.append("-gencode"); cc_flag.append("arch=compute_100,code=sm_100")
            cc_flag.append("-gencode"); cc_flag.append("arch=compute_100,code=compute_100")  # PTX fallback
    ...
    ext_modules.append(
        CUDAExtension(
            name="mamba_ssm.ops.selective_scan_cuda", # selective_scan_cuda
            sources=[
